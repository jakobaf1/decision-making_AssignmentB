...
def make_hindsight_policy(day, occupancy, prices):
    """
    Pre-solves the MILP for the given day and returns a policy function
    that looks up the optimal action for the current hour.
    """
    # Solve once for the full day
    model = build_model_for_day(day, occupancy, prices, data['num_timeslots'])
    solver = SolverFactory('gurobi')
    solver.solve(model)

    # Cache the optimal actions
    optimal_actions = {
        t: {
            "HeatPowerRoom1": value(model.p[0, t]),
            "HeatPowerRoom2": value(model.p[1, t]),
            "VentilationON":  int(round(value(model.v[t])))
        }
        for t in model.T
    }
