...
def environment(policy):
    ...
    for day in range(E):
        ...
        for t in range(data["num_timeslots"]):
            ...
            # humidity dynamics
            new_H = (state["H"]
                    + data["humidity_occupancy_coeff"]*sum(occupancy[r,day,t] for r in [0,1])
            ...
            # update state
            state["T1"], state["T2"], state["H"] = new_T1, new_T2, new_H
            state["low_override_r1"], state["low_override_r2"] = y1, y2
